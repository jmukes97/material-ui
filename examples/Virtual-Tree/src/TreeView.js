...
const TreeView = React.forwardRef(function TreeView(props, ref) {
	...
	if(virtual){
	    const gen = walker ? walker(true) : treeWalker(true)
	    let current = gen.next(true)
	    while(!current.done) {
		if(current.value.id === id) {
		    if(isExpanded(id)) {
			current = gen.next(true)
			return current.value.id
		    }else{
			let skip = current.value.children.length
			if(!current.value.children || !current.value.children.length){
			    current = gen.next(true)
			    return current.value.id
			}
			while(skip !== 0){
			    current = gen.next(true)
			    if(current.value.children && current.value.children.length){
				skip += current.value.children.length
			    }
			    skip = skip - 1
			}
			current = gen.next(true)
	...
	if(virtual){
		...
		if(current.value.id === id) {
		    if(oldNodes.length) {
			const end = oldNodes[oldNodes.length - 1]
			if(end.value.nestingLevel === current.value.nestingLevel){
			    return end.value.id
			}else{
			    for(let i = oldNodes.length - 1; i >= 0; i--){
				if(oldNodes[i].value.nestingLevel === current.value.nestingLevel){
				    return oldNodes[i].value.id
				}
    ...
    function addItem(id, nodeArr){
	const stack = [...treeData.children];
	while (stack.length !== 0) {
	    let node = stack.pop()
	    if(node.id === id){
		if(node.children) {
		    nodeArr.forEach(newNode => {
			let canAdd = true
			node.children.forEach(sibling => {
			    if(newNode.id === sibling.id){
				canAdd = false
			    }
			})
			if(canAdd){
	...
	while (stack.length !== 0) {
	    ...
	    const isOpened = yield  {
			       ...
			       isOpenByDefault: nestingLevel === 0 ? true : false,
			       name,
			       nestingLevel,
			       children,
			   }
	...
	>
		...
		<Tree
		    ...
		    itemSize={itemSize ? itemSize : 25}
		    treeWalker={walker ? walker : treeWalker}
