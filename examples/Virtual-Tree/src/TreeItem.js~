...
const TreeItem = React.forwardRef(function TreeItem(props, ref) {
  const {
	...
	if(expandable && !isExpanded(nodeId)){
	    if(onNodeExpanded){
		if(virtual){
		    const newNodes = onNodeExpanded(event, nodeId)
		    if(Array.isArray(newNodes)){
			if(!addNode){
			    addItem(nodeId, newNodes)
			}else{
			    addNode(nodeId, newNodes)
			}
	...
	switch (key) {
	    ...
	    case 'ArrowRight':
		if(virtual) {
		    if(!isExpanded(nodeId) && expandable) {
			toggleExpansion(event);
			onClick(event)
			if(onNodeExpanded){
			    const newNodes = onNodeExpanded(event, nodeId)
    ...
    }, [children, nodeId, addNodeToNodeMap, virtual]);
